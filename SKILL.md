      ...
      "kcs": [
        {
          ...
          "content": "1-2 句话：是什么 → 为什么重要 → 怎么考；**关键术语用 **术语** 标记**，渲染时墨色加粗（每条 content 至少标 1-3 个核心术语）",
...
| 脚本 | 产物 | 内容 |
|---|---|---|
| render_outline.py | `<课程名>-复习提纲.html` | Claude 式杂志感双栏：暖奶油纸底 + 单一珊瑚 accent，衬线大标题（Georgia）+ 无衬线正文；左 sticky sidebar（两级目录、滚动同步高亮、可拖拽 180–560px、可折叠成 60px 章节色点导航）+ 右正文；章节标题 = 衬线编号 + 细分隔线（零大色块）；知识点挂四色**色点胶囊标签**（必考/重点/高频/了解，无 emoji）；**content 里的 **术语** 渲染为墨色加粗**；importance=must 知识点 = 珊瑚左细线 + 浅珊瑚底 + 左上角「必考考点」微标；顶部 sticky 阅读进度条 + 回顶按钮 + stagger reveal 动效；章末折叠自测（selftests） |
...
| render_graph.py | `<课程名>-知识图谱.html` | Claude 式杂志感（与复习提纲同源配色/字体）：暖奶油纸底 + 单一珊瑚 accent，顶部极淡珊瑚光晕；居中 hero（eyebrow + 衬线大标题 Georgia + 珊瑚细分隔线 + 说明文字）；左章节节点（暖米底 + 米线边 + 衬线珊瑚编号 + 墨色无衬线标题）/ 右知识点节点（四色低饱和胶囊：必考珊瑚 / 重点金棕 / 高频蓝灰 / 了解灰绿，无 emoji 标签）；`deps` 画米色虚线依赖（带箭头），`is_hub` 加珊瑚 ★ 星标；图谱直接铺在 body 背景上（无盒子），`graph-stage` 负 margin 吃掉 wrap 左右留白；鼠标拖拽平移 + 滚轮缩放，移动端单指拖拽 + 双指 pinch 缩放，悬停 tooltip 去掉 markdown 加粗标记；hover 节点加柔投影 |
