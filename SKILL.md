...
8. **填空题的空位要对齐答案**：题干里每个空位写 `___`（3 个以上下划线）或 `【】`，**空位数必须等于 `answer` 数组长度**——判分是"输入框数 == 答案数 且逐位比对"，空位少了渲染器会自动在题干下方补足输入框（能判分，但题干没标出空在哪，作答体验差），空位多了则永远判错。写完数一遍。
...
- `fill`：字符串数组，按空顺序，如 `["TCP", "三次握手"]`；题干空位用 `___`（3 个以上下划线）或 `【】` 占位，**空位数必须等于答案数**（见 Phase 3 规则 8）
...
| render_quiz.py | `<课程名>-复习题.html` | 刷题 App 风：深色 header（右上角 ▲/▼ 可折叠顶部栏）+ 统计条/进度条 + 题型 tab（固定 单选/多选/判断/填空/客观题/主观题，客观在前主观在后，0 道也占位）+ **章节 tab（显示真实章节名；questions.json 缺 label 时按 id 从 knowledge_skeleton 补齐，不再显示 ch1/ch2；章节名多放不下时鼠标滚轮可横向滚动、滚动条保持隐藏）** + 刷题模式（全部题目/随机顺序/极速刷题/未作答/错题本；**极速刷题**一次把当前筛选题目整页列出、下滑就地作答、顶栏常驻，不必点“下一题”）；单题视图逐题判对错、高亮答案、显示解析与易错点（单选/判断点选项即自动判分；**填空题空位兼容 `___` 与中文 `【】`，空位不足或题干没写空位时自动在题干下方补足输入框，使输入框数不少于答案数**）；做错的题自动进「错题本」可重刷、错题本旁有「导出错题」按钮可一键下载仅含错题的离线 HTML（受去AI影响）；顶部深色栏右侧提供「去AI/已去AI」切换（一键隐藏/恢复 AI 生成题，有/无 AI 时按钮状态不同）与「掌握度报告」入口；主观题一键看参考答案；「掌握度报告」按知识点聚合得分率标薄弱、以弹窗形式展示；页面隐藏滚动条但保留滚动；Ctrl+P 打印时全部题目+答案+解析展开 |
| render_graph.py | `<课程名>-知识图谱.html` | Claude 式杂志感（与知识清单同源配色/字体），**两级放射 · 总览 + 点章展开**：暖奶油纸底 + 珊瑚 accent，居中 hero；图谱放进固定高度的圆角舞台（内容不把页面撑长），**总览 = 中央「课程」胶囊 + 一圈章节节点**（编号 + 标题，辐线连中心，一屏扫结构；**编号与标题首行共用基线、画在胶囊内**，常驻可见，聚焦某章时随该章一起变淡）；**点某章 → 该章知识点朝外展开成扇形**（章→知识点浅线 + 章内 `deps` 虚线依赖，`is_hub` 加 ★；画布自动放大聚焦到这一章，其余章节变淡），点空白/课程/再点同章回总览；知识点胶囊四色重要度（必考珊瑚/重点金棕/高频蓝灰/了解灰绿），悬停 tooltip 显示内容并去掉 markdown 加粗；拖拽平移、滚轮缩放、双指 pinch、按 0 回总览；知识点布局每章做去重叠排版，间距宽松；图谱区右上角有「全屏 / 退出全屏」按钮，方便整屏看图谱 |
