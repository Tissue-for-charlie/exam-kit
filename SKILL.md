...
| render_quiz.py | `<课程名>-复习题.html` | 刷题 App 风：深色 header（右上角 ▲/▼ 可折叠顶部栏）+ 统计条/进度条 + 题型 tab（固定 单选/多选/判断/填空/客观题/主观题，客观在前主观在后，0 道也占位）+ **章节 tab（显示真实章节名；questions.json 缺 label 时按 id 从 knowledge_skeleton 补齐，不再显示 ch1/ch2；章节名多放不下时鼠标滚轮可横向滚动、滚动条保持隐藏）** + 刷题模式（全部题目/随机顺序/极速刷题/未作答/错题本；**极速刷题**一次把当前筛选题目整页列出、下滑就地作答、顶栏常驻，不必点“下一题”）；单题视图逐题判对错、高亮答案、显示解析与易错点（单选/判断点选项即自动判分；**填空题空位兼容 `___` 与中文 `【】`，空位不足或题干没写空位时自动在题干下方补足输入框，保证输入框数恒等于答案数**）；做错的题自动进「错题本」可重刷、错题本旁有「导出错题」按钮可一键下载仅含错题的离线 HTML（受去AI影响）；顶部深色栏右侧提供「去AI/已去AI」切换（一键隐藏/恢复 AI 生成题，有/无 AI 时按钮状态不同）与「掌握度报告」入口；主观题一键看参考答案；「掌握度报告」按知识点聚合得分率标薄弱、以弹窗形式展示；页面隐藏滚动条但保留滚动；Ctrl+P 打印时全部题目+答案+解析展开 |
